...
def chirp(L, f0=-25e3, cr=160e3, sr=50e3, use_numpy=False):
    ...
    if use_numpy:
        chirp = np.exp(1j * np.mod(dphase, 2 * np.pi)) * \
            np.exp(1j * 2 * np.pi * f0 * tv)
    else:
...
def spectrogram(x, window=1024, step=512, wf=ss.hann(1024), n_oversample=13):
    ...
    substep_idx = np.arange(n_oversample, dtype=int) - \
        int((n_oversample - 1) / 2)

    S = np.zeros([n_spec, window], dtype=np.float64)
...
#            print("%d %d"%(i0,step))
            ...
            if (i0 >= 0) and (i1 < len(x)):
                freq_pwr = np.abs(np.fft.fftshift(
                    np.fft.fft(wf * x[i0:i1])))**2.0
                std_est = np.nanmedian(np.abs(freq_pwr - n.nanmedian(freq_pwr)))
#                print(std_est)
                if std_est > 0:
                    # *np.abs(np.fft.fftshift(np.fft.fft(wf*x[i0:i1])))**2.0
                    S[i, :] += (1.0 / (std_est**2.0)) * freq_pwr
...
def decimate(x, dec):
    ...
    idx = np.arange(Nout, dtype=int) * int(dec)
